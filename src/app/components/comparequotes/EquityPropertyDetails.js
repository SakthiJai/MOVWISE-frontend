...
export default function EquityPropertyDetails({quote, servicData, companydata, cardid, taxDetails, giftvalue, language, lenders, handleprice, hide }) {
  const selectedEquityItem = companydata?.find(
    (item) =>
      item?.conveying_details?.conveying_id ==
      (cardid ?? quote?.conveying_details?.conveying_id)
  );
  const activeItem = selectedEquityItem || quote;
  const equityItems = activeItem ? [activeItem] : [];
    ...
    <div className="py-2 font text-sm">
      ...
      <div className="remortgage-pdf-summary pdf-only" style={{ padding: "12px 0" }}>
        <p style={{ fontSize: "13px", lineHeight: "1.8" }}>
          {servicData?.country || "--"}, equity transfer for property value <strong>£{servicData?.property_values || "--"}</strong>, <strong>{servicData?.leasehold_or_free || "--"}</strong>, {servicData?.property_type || "--"}, Buy to let:<strong>{servicData?.buy_to_let || "--"}</strong>, Obtaining Mortgage: <strong>
